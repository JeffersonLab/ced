...
public final class CentralXYView extends CedXYView {
	...
	}	if(isDisplayed(CedDisplayOption.ACCUMULATION))drawAccumulatedMarkers(g,c,Detector.BST);else if(isDisplayed(CedDisplayOption.RAW_DATA))for(AdcHit h:data.adcHits())if(h.detector()==Detector.BST)drawBSTMarker(g,c,h,false);}
	...
	private void drawBMT(Graphics2D g,IContainer c){for(int layer=1;layer<=BMTGeometry.LAYER_COUNT;layer++){BMTGeometry.Layer info=bmt.layer(layer);for(int sector=0;sector<3;sector++){
		...
		}}	if(isDisplayed(CedDisplayOption.ACCUMULATION))drawAccumulatedMarkers(g,c,Detector.BMT);else if(isDisplayed(CedDisplayOption.RAW_DATA))for(AdcHit h:data.adcHits())if(h.detector()==Detector.BMT)drawBMTMarker(g,c,h,false);}
	private void drawBMTMarker(Graphics2D g,IContainer c,AdcHit h,boolean recon){if(h.layer()<1||h.layer()>6)return;BMTGeometry.Layer info=bmt.layer(h.layer());double f=Math.max(0,Math.min(1,(h.component()-0.5)/Math.max(1,info.stripCount())));int geoSector=bmtSector(h.sector());double phi=info.phiMinDeg()+120*geoSector+f*(info.phiMaxDeg()-info.phiMinDeg());Point p=screen(c,info.radiusMm()/10*Math.cos(Math.toRadians(phi)),info.radiusMm()/10*Math.sin(Math.toRadians(phi)));markers.put(h,p);drawMarker(g,p,recon?Color.RED:channelColor(h));}

	private void drawCTOF(Graphics2D g,IContainer c){for(int paddle=1;paddle<=CTOFGeometry.PADDLE_COUNT;paddle++){Element e=new Element(Detector.CTOF,1,1,paddle,0);Polygon p=polygon(c,ctof.quad(paddle),.1);polygons.put(e,p);fill(g,p,elementColor(e));}}
	...
	private Color elementColor(Element e){if(isDisplayed(CedDisplayOption.ACCUMULATION)){int max=accumulation.maximumCount();int n=accumulation.count(e.detector,e.sector,e.layer,e.component,e.order);return n==0||max==0?EMPTY:ScientificColorMap.TURBO.colorAt((double)n/max);}if(!isDisplayed(CedDisplayOption.RAW_DATA)||data.maximumAdc()==0)return EMPTY;int max=0;for(AdcHit h:data.adcHits())if(matches(e,h))max=Math.max(max,h.adc());return max==0?EMPTY:ScientificColorMap.TURBO.colorAt((double)max/data.maximumAdc());}
	private Color channelColor(AdcHit h){if(!isDisplayed(CedDisplayOption.RAW_DATA)||data.maximumAdc()==0)return EMPTY;return ScientificColorMap.TURBO.colorAt((double)h.adc()/data.maximumAdc());}
	...
	@Override public void getFeedbackStrings(IContainer c,Point sp,Point2D.Double wp,List<String> feedback){super.getFeedbackStrings(c,sp,wp,feedback);for(var entry:polygons.entrySet())if(entry.getValue().contains(sp)){Element e=entry.getKey();feedback.add("$cyan$"+e.detector+" sector "+e.sector+" layer "+e.layer+" component "+e.component);for(AdcHit h:data.adcHits())if(matches(e,h))feedback.add(String.format("$cyan$adc %d time %.3f order %d",h.adc(),h.time(),h.order()));if(isDisplayed(CedDisplayOption.ACCUMULATION))feedback.add("$cyan$occupancy "+accumulation.count(e.detector,e.sector,e.layer,e.component,e.order)+" / "+accumulation.eventCount()+" events");break;}for(var entry:markers.entrySet())if(entry.getValue().distance(sp)<=9)addMarkerFeedback(entry.getKey(),feedback);}
	...
	private static void drawArc(Graphics2D g,IContainer c,double radius,double start,double extent,Color color){Point center=screen(c,0,0),edge=screen(c,radius,0);double rp=Math.abs(edge.x-center.x);Arc2D arc=new Arc2D.Double(center.x-rp,center.y-rp,2*rp,2*rp,start,extent,Arc2D.OPEN);g.setColor(color);g.setStroke(new BasicStroke(4f));g.draw(arc);g.setColor(Color.DARK_GRAY);g.setStroke(new BasicStroke(1f));g.draw(arc);}
