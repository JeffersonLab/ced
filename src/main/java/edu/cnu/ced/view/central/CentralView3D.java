 ...
 * (48 paddles), and BST/BMT (six barrel-tracker layers each) -- every
 * panel colored by this event's ADC hit where present -- plus BST/BMT
 * reconstructed crosses and every category of reconstructed/Monte Carlo
 * track, each swum on demand through the same shared {@link
 * SwimTrajectoryCache} every 2D view already uses. Second of CED's seven
 * 3D views (after FTCal 3D), built on the same shared {@code
...
public final class CentralView3D extends CedView3D {
	...
	public CentralView3D(CNDGeometry cnd, CTOFGeometry ctof, BSTGeometry bst, BMTGeometry bmt,
		...
		CentralPanel3D panel = (CentralPanel3D) cedPanel3D();
		panel.setGeometry(cnd, ctof, bst, bmt);
	...
	protected void eventChanged(EventNavigationState state) {
		((CentralPanel3D) cedPanel3D()).setEventData(state.snapshot());
	}
}
